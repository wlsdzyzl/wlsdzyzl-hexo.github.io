...
基函数$a\_{kl}\[m,n\]$以及$a\_{kl}^\*\[m,n\]$满足：
...
f\_{e}(x)=\\left\\{\\begin{array}{ll}{f(x)} & {0 \\leq x \\leq A-1} \\\\ {0} & {A \\leq x \\leq P}\\end{array}\\right. g\_{e}(x)=\\left\\{\\begin{array}{ll}{g(x)} & {0 \\leq x \\leq B-1} \\\\ {0} & {B \\leq x \\leq P}\\end{array}\\right.
